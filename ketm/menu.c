...
{
	char *m00="SOUND VOLUME";
	char *m01="DIFFICULTY";
	char *m02="KEY CONFIGURATION";
	char *m03="DISPLAY: FULLSCREEN";
	char *m13="DISPLAY: WINDOW";
	char *m04="FRAMERENDER: MAX";
	char *m14="FRAMERENDER: FIXED";
	char *m05="SHOW FPS";
	char *m15="HIDE FPS";
	char *m06="BACK TO MAIN MENU";
	...
	if(!optionmenu.select_finish) {
		...
		switch(optionmenu.active_item) {
			...
			case 1:	/* difficulty */
				...
				break;
			case 2: /* Key configuration */
				error(ERR_WARN,"not yet...");
				newstate(ST_MENU,MEN_OPTION,1);
				break;
			case 3: /* toggle fullscreen/window */
				...
				newstate(ST_MENU,MEN_OPTION,1);
				break;
			case 4: /* toggle fps-method */
				toggle_fps();
				newstate(ST_MENU,MEN_OPTION,1);
				break;
			case 5: /* show/hide fps display */
...
void genericmenu_init(char *options[], MENU *m, int fadeout, int timeout)
{
	...
	for(i=0;i<m->nopt;i++) {
		for(j=0;j<5;j++){
			...
			d->i1=(screen->h/2)-((m->opts[i][j]->h+5)*m->nopt/2)+i*(m->opts[i][j]->h+5);
			d->i2=0;
			d->i3=0;
		}
	}
