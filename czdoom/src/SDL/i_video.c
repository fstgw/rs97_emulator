...
void I_UpdateVideoMode(void)
{
  ...
  if (V_GetMode() == VID_MODEGL) {
    ...
    ScreenSurface = SDL_SetVideoMode(320, 480, gl_colorbuffer_bits, init_flags);
    screen = SDL_CreateRGBSurface(SDL_SWSURFACE, SCREENWIDTH, SCREENHEIGHT, gl_colorbuffer_bits, 0, 0, 0, 0);
  } else {
    //screen = SDL_SetVideoMode(SCREENWIDTH, SCREENHEIGHT, V_GetNumPixelBits(), init_flags);
    ScreenSurface = SDL_SetVideoMode(320, 480, V_GetNumPixelBits(), init_flags);
    screen = SDL_CreateRGBSurface(SDL_SWSURFACE, SCREENWIDTH, SCREENHEIGHT, gl_colorbuffer_bits, 0, 0, 0, 0);
  }
