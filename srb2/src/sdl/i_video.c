...
static void SDLSetMode(INT32 width, INT32 height, INT32 bpp, Uint32 flags)
{
	...
	// dirty fix by steward for retrogame
	/*
...
void I_FinishUpdate(void)
{
	...
	if (render_soft == rendermode && screens[0])
	{
		...
		{
			blited = 0;//SDL_BlitSurface(bufSurface,NULL,vidSurface,dstrect);
			if(SDL_MUSTLOCK(vidSurface)) SDL_LockSurface(vidSurface);
			int x, y;
			uint8_t *s = bufSurface->pixels;
			uint16_t *d = (uint16_t*)vidSurface->pixels + (20 * 2 * 320);
			for(y=0; y<200; y++){
				for(x=0; x<320; x++){
					*d++ = (Uint16)SDL_MapRGB(vidformat, localPalette[*s].r,localPalette[*s].g,localPalette[*s].b);
					s+= 1;
				}
				d+= 320;
			}
		...
		else if (vid.bpp == 1 && !vid.direct)
		{
			...
			vW = (Uint16)((vidSurface->pitch << 1) - vidSurface->w*vidformat->BytesPerPixel);
			if (vidSurface->h > vid.height)
