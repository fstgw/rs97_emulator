...
enum {m_none, m_main, m_singleplayer, m_load, m_save, m_multiplayer, m_setup, m_namemaker, m_net, m_options, m_video, m_keys, m_help, m_quit, m_serialconfig, m_modemconfig, m_lanconfig, m_gameoptions, m_search, m_slist} m_state;
...
void M_Menu_NameMaker_f (void) {
	...
	m_state = m_namemaker;
	m_entersound = true;
	Q_strncpyz (command_name, command.string, sizeof(command_name));
...
void M_NameMaker_Draw (void) {
	...
	M_DrawTextBox (92, 32, 15, 1);

	if (command_kayboard == 1) {
		M_Print (20, 40, "Command");
		M_PrintWhite (104, 40, command_name);
	} else {
		M_Print (20, 40, "Your name");
		M_PrintWhite (104, 40, namemaker_name);
	}

	M_DrawTextBox (16, 48, 33, 18);

	M_DrawTextBox (240, 32, 6, 1);
	M_Print (252, 40, "Enter");

	for (y = 0; y < NAMEMAKER_TABLE_SIZE ;y++)
		for (x=0 ; x<NAMEMAKER_TABLE_SIZE ; x++)
			M_DrawCharacter (36 + (16 * x), 64 + (8 * y), NAMEMAKER_TABLE_SIZE * y + x);

	if (namemaker_cursor_y == NAMEMAKER_TABLE_SIZE)
		M_DrawCharacter (236, 40, 12 + ((int)(realtime*4)&1));
	else
		M_DrawCharacter (28 + 16*namemaker_cursor_x, 64 + 8*namemaker_cursor_y, 12 + ((int)(realtime*4)&1));
}
	...
	case K_CTRL:
		if (namemaker_cursor_y == NAMEMAKER_TABLE_SIZE) {
			...
			if (command_kayboard == 1) {
				...
				if (l < 15) {
					command_name[l] = NAMEMAKER_TABLE_SIZE * namemaker_cursor_y + namemaker_cursor_x;
					command_name[l+1] = 0;
				}
			} else {
...
void M_AdjustSliders (int dir) {
	...
	case 0:	// Load Configs
		Cvar_SetValue ("cl_config", cl_config.value + dir*1);

		if (cl_config.value > 2)
			Cvar_SetValue ("cl_config", 0);
		if (cl_config.value == 0)
			Cbuf_InsertText ("exec config1.cfg\n");
		if (cl_config.value == 1) 
			Cbuf_InsertText ("exec config2.cfg\n");
		if (cl_config.value == 2)
			Cbuf_InsertText ("exec userconfig.cfg\n");
		if (cl_config.value < 0)
			Cvar_SetValue ("cl_config", 2);
		break;
	case 4:	// screen size
	...
	case 10:	// lookstrafe
		...
		break;
	case 11:	// Show FPS
		Cvar_SetValue ("cl_showfps", (float)!cl_showfps.value);
		break;
		/*
...
void M_Options_Draw (void)
{
	...
	M_Print (16, 32, "       Setting");
	if (cl_config.value == 0)
		M_Print (140, 32, "Config 1");
	if (cl_config.value == 1)
		M_Print (140, 32, "Config 2");
	if (cl_config.value == 2)
		M_Print (140, 32, "Custom Config");

	M_Print (16, 40, "      Controls");
	//M_Print (16, 32, "    Customize controls");
	M_Print (16, 48, "       Command");
	M_Print (16, 56, "      Defaults");
	...
	M_DrawCheckbox (140/*220*/, 104, (int)lookspring.value);

	M_Print (16, 112, "    Lookstrafe");
	M_DrawCheckbox (140/*220*/, 112, (int)lookstrafe.value);

	M_Print (16, 120, "      Show FPS");
	M_DrawCheckbox (140/*220*/, 120, (int)cl_showfps.value);
...
// Config Info
	if (options_cursor == 3) {
		if (cl_config.value == 2)
			M_Print (48, 212, "Press A-Button to set Default");
		else
			M_Print (40, 212, "Can use only with Custom Config");
	}
}
...
void M_ServerList_Draw (void) {
	...
	if (!slist[0].server) {
		...
		M_Print (40, MENU_Y + 192, "Press Y-Button to add a server");
		return;
	}
	...
	for (serv = slist_mins, line = 1 ; serv <= slist_maxs && serv < MAX_SERVER_LIST && slist[serv].server ; serv++, line++)
		...
		M_Print (40, MENU_Y + 192, "Press Y-Button to add a server");
		M_Print (28, MENU_Y + 200, "Press X-Button to remove a server");
