...
void SetVideo(unsigned char mode)
{
	...
	#else
		actualScreen = SDL_SetVideoMode(screen_scale.w_display, /*screen_scale.h_display*/480, BITDEPTH_OSWAN, flags);
	#endif
...
void Set_DrawRegion(void)
{
	...
	else if (GameConf.m_ScreenRatio == 0)
	{
		...
		screen_to_draw_region.offset_y = ((/*actualScreen->h*/240 - SYSVID_HEIGHT)/2); 
#endif
	}
}
