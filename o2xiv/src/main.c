...
fixed scale = int_to_fixed(1),
      scale_inv = int_to_fixed(1); // multiplicative inverse
...
void redraw( void ) {
	...
	// scale partial screen
	} else if (pan_rect.x != drawn_rect.x || pan_rect.y != drawn_rect.y) {
		//  printf("pan (partial)\tx %d\ty %d\tx %d\ty %d\n", pan_rect.x, pan_rect.y, pan_rect.x + min(pan_rect.w, scaled->w), pan_rect.y + min(pan_rect.h, scaled->h)); fflush(stdout);
		
		// shift already drawn image
		SDL_Rect move_rect = { drawn_rect.x - pan_rect.x, drawn_rect.y - pan_rect.y, 0, 0 };
		// printf("move\tx %d\ty %d\tx %d\ty %d\n", move_rect.x, move_rect.y, move_rect.x + move_rect.w, move_rect.y + move_rect.h); fflush(stdout);
		SDL_BlitSurface(scaled, NULL, scaled, &move_rect);
		
		if SDL_MUSTLOCK(image) SDL_LockSurface(image);
		if SDL_MUSTLOCK(scaled) SDL_LockSurface(scaled);
		
		// scale the rest
		if (pan_rect.x < drawn_rect.x) { // left
			SDL_Rect draw_rect = { 0, 0, drawn_rect.x - pan_rect.x, scaled_rect.h };
#ifdef TARGET_GP2X
			draw_rect.w++; // cover over ugly SDL bug
#endif
			// printf("draw\tx %d\ty %d\tx %d\ty %d\n", draw_rect.x, draw_rect.y, draw_rect.x + draw_rect.w, draw_rect.y + draw_rect.h); fflush(stdout);
			fixed_scale(image, scaled, pan_rect.x * scale_inv, pan_rect.y * scale_inv, &draw_rect, scale, scale_inv);
		}
		if (pan_rect.x > drawn_rect.x) { // right
			SDL_Rect draw_rect = { scaled_rect.w - (pan_rect.x - drawn_rect.x), 0, pan_rect.x - drawn_rect.x, scaled_rect.h };
			// printf("draw\tx %d\ty %d\tx %d\ty %d\n", draw_rect.x, draw_rect.y, draw_rect.x + draw_rect.w, draw_rect.y + draw_rect.h); fflush(stdout);
			fixed_scale(image, scaled, (pan_rect.x + draw_rect.x) * scale_inv, pan_rect.y * scale_inv, &draw_rect, scale, scale_inv);
		}
		if (pan_rect.y < drawn_rect.y) { // top
			SDL_Rect draw_rect = { 0, 0, scaled_rect.w, drawn_rect.y - pan_rect.y };
			// printf("draw\tx %d\ty %d\tx %d\ty %d\n", draw_rect.x, draw_rect.y, draw_rect.x + draw_rect.w, draw_rect.y + draw_rect.h); fflush(stdout);
			fixed_scale(image, scaled, pan_rect.x * scale_inv, pan_rect.y * scale_inv, &draw_rect, scale, scale_inv);
#ifdef TARGET_GP2X
			draw_rect.h++; // cover over ugly SDL bug
#endif
		}
		if (pan_rect.y > drawn_rect.y) { // bottom
			SDL_Rect draw_rect = { 0, scaled_rect.h - (pan_rect.y - drawn_rect.y), scaled_rect.w, pan_rect.y - drawn_rect.y };
			// printf("draw\tx %d\ty %d\tx %d\ty %d\n", draw_rect.x, draw_rect.y, draw_rect.x + draw_rect.w, draw_rect.y + draw_rect.h); fflush(stdout);
			fixed_scale(image, scaled, pan_rect.x * scale_inv, (pan_rect.y + draw_rect.y) * scale_inv, &draw_rect, scale, scale_inv);
		}
