...
void                   ecran_joueurRaffraichissement(Ecran_joueur* ecran)
{
  ...
  if (joueurLireVie(ecran->j_courant) >1)   
     sprintf(texte, "Vies: %d Diamants: %d Temps: %d    ", joueurLireVie(ecran->j_courant), niveauLireNombreDiamantRestant(ecran->niveau), niveauLireTempsRestant(ecran->niveau));
  else
     sprintf(texte, "Vie: %d Damants: %d Temps: %d     ", joueurLireVie(ecran->j_courant), niveauLireNombreDiamantRestant(ecran->niveau), niveauLireTempsRestant(ecran->niveau));
