...
/*============================================================ */
#ifdef GCW0
void osd_customize_inputport_rotated_defaults(unsigned int rotation, struct ipd *defaults)
{
	...
	if (rotation == ROT90)
	{
	    while (idef->type != IPT_END)
	    {
		if (idef->type == IPT_JOYSTICK_UP) seq_set_1(&idef->seq, KEYCODE_RIGHT);
		if (idef->type == IPT_JOYSTICK_DOWN) seq_set_1(&idef->seq, KEYCODE_LEFT);
		if (idef->type == IPT_JOYSTICK_LEFT) seq_set_1(&idef->seq, KEYCODE_UP);
		if (idef->type == IPT_JOYSTICK_RIGHT) seq_set_1(&idef->seq, KEYCODE_DOWN);

		if (idef->type == IPT_UI_UP) seq_set_1(&idef->seq, KEYCODE_RIGHT);
		if (idef->type == IPT_UI_DOWN) seq_set_1(&idef->seq, KEYCODE_LEFT);
		if (idef->type == IPT_UI_LEFT) seq_set_1(&idef->seq, KEYCODE_UP);
		if (idef->type == IPT_UI_RIGHT) seq_set_1(&idef->seq, KEYCODE_DOWN);
	...
	else
	{
	    while (idef->type != IPT_END)
	    {
		if (idef->type == IPT_JOYSTICK_UP) seq_set_1(&idef->seq, KEYCODE_LEFT);
		if (idef->type == IPT_JOYSTICK_DOWN) seq_set_1(&idef->seq, KEYCODE_RIGHT);
		if (idef->type == IPT_JOYSTICK_LEFT) seq_set_1(&idef->seq, KEYCODE_DOWN);
		if (idef->type == IPT_JOYSTICK_RIGHT) seq_set_1(&idef->seq, KEYCODE_UP);

		if (idef->type == IPT_UI_UP) seq_set_1(&idef->seq, KEYCODE_LEFT);
		if (idef->type == IPT_UI_DOWN) seq_set_1(&idef->seq, KEYCODE_RIGHT);
		if (idef->type == IPT_UI_LEFT) seq_set_1(&idef->seq, KEYCODE_DOWN);
		if (idef->type == IPT_UI_RIGHT) seq_set_1(&idef->seq, KEYCODE_UP);
		idef++;
	    }
	}
}
...
void osd_customize_inputport_defaults(struct ipd *defaults)
{
	...
	while (idef->type != IPT_END)
	{
	    if (idef->type == IPT_UI_SELECT) seq_set_1(&idef->seq, KEYCODE_LCONTROL);
	    if (idef->type == IPT_UI_CANCEL) seq_set_3(&idef->seq, KEYCODE_ESC, KEYCODE_TAB, KEYCODE_BACKSPACE);
	    if (idef->type == IPT_UI_PAUSE) seq_set_4(&idef->seq, KEYCODE_TAB, KEYCODE_BACKSPACE, CODE_NOT, KEYCODE_ESC);
	    if (idef->type == IPT_UI_CONFIGURE) seq_set_2(&idef->seq, KEYCODE_ENTER, KEYCODE_TAB);
	    if (idef->type == IPT_UI_SHOW_FPS) seq_set_2(&idef->seq, KEYCODE_ENTER, KEYCODE_BACKSPACE);
	    if (idef->type == IPT_START1) seq_set_2(&idef->seq, KEYCODE_ESC, KEYCODE_TAB);
	    if (idef->type == IPT_COIN1)  seq_set_2(&idef->seq, KEYCODE_ESC, KEYCODE_BACKSPACE);
	    if (idef->type == IPT_UI_SAVE_STATE) seq_set_2(&idef->seq,KEYCODE_ENTER,KEYCODE_LEFT);
	    if (idef->type == IPT_UI_LOAD_STATE) seq_set_2(&idef->seq,KEYCODE_ENTER,KEYCODE_RIGHT);
	    if (idef->type == IPT_UI_SNAPSHOT) seq_set_2(&idef->seq,KEYCODE_ENTER,KEYCODE_DOWN);
	    if (idef->type == (IPT_BUTTON1 | IPF_PLAYER1)) seq_set_1(&idef->seq,KEYCODE_LCONTROL);
	    if (idef->type == (IPT_BUTTON2 | IPF_PLAYER1)) seq_set_1(&idef->seq,KEYCODE_LALT);
	    if (idef->type == (IPT_BUTTON3 | IPF_PLAYER1)) seq_set_1(&idef->seq,KEYCODE_LSHIFT);
	    if (idef->type == (IPT_BUTTON4 | IPF_PLAYER1)) seq_set_1(&idef->seq,KEYCODE_SPACE);
	    /* find the next one */
	    idef++;
	}
