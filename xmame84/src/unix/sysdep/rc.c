...
#ifdef GCW0
	    {
	      if (strcmp(option[i].name, "stdout-file") == 0)
	      {
		if(*(FILE **)option[i].dest != stdout)
		  fclose(*(FILE **)option[i].dest);
	      }
	      else if (strcmp(option[i].name, "stderr-file") == 0)
	      {
		if(*(FILE **)option[i].dest != stderr)
		  fclose(*(FILE **)option[i].dest);
	      }
	      else
		  fclose(*(FILE **)option[i].dest);
