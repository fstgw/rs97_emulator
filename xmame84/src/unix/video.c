...
struct rc_option video_opts[] = {
   ...
   { "bpp",		"b",			rc_int,		&options.color_depth,
     ...
     "Specify the colordepth the core should render, one of: auto(0), 8, 15, 16, 32" },
...
/* Disabled artwork, backgrop, overlay and bezels for GCW0 */
...
#ifdef GCW0
     "320x240",		0,			0,		video_verify_vectorres,
#else
...
void osd_video_initpre()
{
	...
	if (video_ror)
	{
		...
		orientation ^= ROT90;
#ifdef GCW0
		// Rotate controls so it's the same orientation as the video
		extern struct ipd inputport_defaults[];
		osd_customize_inputport_rotated_defaults(ROT90, inputport_defaults);
#endif
	}
	...
	if (video_rol)
	{
		...
		// Rotate controls so it's the same orientation as the video
		extern struct ipd inputport_defaults[];
		osd_customize_inputport_rotated_defaults(ROT270, inputport_defaults);
#endif
	}
...
printf("osd_create_display():visual_width=%u visual_height=%u\n", visual_width, visual_height);
    }
	else
	{
#endif
	...
	fprintf(stderr_file, "viswidth = %d, visheight = %d, visstartx= %d,"
			"visstarty= %d blit_swapxy= %d blit_flipx= %d blit_flipy= %d\n",
			visual_width, visual_height, visual.min_x, visual.min_y,
			blit_swapxy, blit_flipx, blit_flipy);
#endif

#ifdef GCW0
	if (sysdep_create_display(visual_width, visual_height, bitmap_depth, using_15bpp_rgb_direct) != OSD_OK)
#else
	...
	if (rgb_components)
	{
		...
		{
#ifdef DIRECT_RGB565
			rgb_components[0] = 0xf800;
			rgb_components[1] = 0x07e0;
...
#ifdef GCW0
		if (sysdep_create_display(visual_width, visual_height, new_depth, using_15bpp_rgb_direct) != OSD_OK)
#else
...
printf("Game Visual %dx%d Box %u.%u %u.%u\n", visual_width, visual_height, visual.min_x, visual.min_y, visual.max_x, visual.max_y);
}
