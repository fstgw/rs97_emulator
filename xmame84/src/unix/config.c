...
extern char frontendname[PATH_MAX];
#endif
#ifdef SPEEDHACK
...
static struct rc_option opts[] = {
   /* name, shortname, type, dest, deflt, min, max, func, help */
...
#ifdef SPEEDHACK
	{ "speedhack", "sh", rc_int, &speedhack, "0", 0, 3, NULL, "Enable speedhacks 1=90% CPU, 2=80% CPU, 3=70% CPU underclock" },
#endif
	...
	{ "frontend", "fe", rc_string, &frontend, NULL, 0, 0, NULL, "Frontend executable to run on exit" },
...
int config_init (int argc, char *argv[])
{
	...
	/* get the homedir */
	if(!(home_dir = getenv("XMAMEROOT")))
	{
		fprintf(stderr, "Could not get XMAMEROOT env var\n");
		return OSD_NOT_OK;
	}

	/* check that the required dirs exist, and create them if necessary */
	snprintf(buffer, BUF_SIZE, "%s", home_dir);
	if (check_and_create_dir(buffer))
		return OSD_NOT_OK;

	snprintf(buffer, BUF_SIZE, "%s/%s", home_dir, "cfg");
	if (check_and_create_dir(buffer))
		return OSD_NOT_OK;

	snprintf(buffer, BUF_SIZE, "%s/%s", home_dir, "mem");
	if (check_and_create_dir(buffer))
		return OSD_NOT_OK;

	snprintf(buffer, BUF_SIZE, "%s/%s", home_dir, "sta");
	if (check_and_create_dir(buffer))
		return OSD_NOT_OK;

	snprintf(buffer, BUF_SIZE, "%s/%s", home_dir, "nvram");
	if (check_and_create_dir(buffer))
		return OSD_NOT_OK;

	snprintf(buffer, BUF_SIZE, "%s/%s", home_dir, "diff");
	if (check_and_create_dir(buffer))
		return OSD_NOT_OK;

	snprintf(buffer, BUF_SIZE, "%s/%s", home_dir, "rc");
	if (check_and_create_dir(buffer))
		return OSD_NOT_OK;

	snprintf(buffer, BUF_SIZE, "%s/%s", home_dir, "hi");
	if (check_and_create_dir(buffer))
		return OSD_NOT_OK;

	snprintf(buffer, BUF_SIZE, "%s/%s", home_dir, "inp");
	if (check_and_create_dir(buffer))
		return OSD_NOT_OK;
	...
	if(loadconfig)
	{
		snprintf(buffer, BUF_SIZE, "%s/%src", home_dir, NAME);
	...
	if (loadconfig)
	{
		snprintf(buffer, BUF_SIZE, "%s/rc/%src", home_dir, drivers[game_index]->name);
		if(rc_load(rc, buffer, 1, 1))
			return OSD_NOT_OK;
	}
...
#if GCW0
	/* Set the filename of the executable to run on exit */
	if (frontend)
	{
	    strcpy(frontendname, frontend);
	}
#endif
#ifdef SPEEDHACK
	/* Underclock CPUs */
	if (speedhack)
	{
	    ...
	    {
		cpunum_set_clockscale(i, cpu_underclock);
	    }
	}
#endif
...
void show_usage(void) 
{
	...
	fprintf(stdout_file, "\n"
			...
			"GCW-Zero port by Anthony Staude (AKA Slaanesh)\n", NAME);
}
