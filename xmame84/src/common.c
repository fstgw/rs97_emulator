...
void save_screen_snapshot_as(mame_file *fp, struct mame_bitmap *bitmap)
{
...
#ifdef GCW0
	{
		...
		if (orientation == ROT270)
		{
			if (Machine->drv->video_attributes & VIDEO_TYPE_VECTOR)
			{
			    sizey = options.vector_width;
			    sizex = options.vector_height;
			}
			else
			{
			    sizey = bounds.max_x - bounds.min_x + 1;
			    sizex = bounds.max_y - bounds.min_y + 1;
			}

			scaley = (Machine->drv->video_attributes & VIDEO_PIXEL_ASPECT_RATIO_2_1) ? 2 : 1;
			scalex = (Machine->drv->video_attributes & VIDEO_PIXEL_ASPECT_RATIO_1_2) ? 2 : 1;
			...
			if (copy)
			{
				...
				case 8:
					for (y = 0;y < copy->height;y++)
					{
						...
						{
							((UINT8 *)copy->line[copy->height - y -1])[x] = ((UINT8 *)bitmap->line[sy+(x/scalex)])[sx +(y/scaley)];
						}
					}
					break;
				case 15:
				case 16:
					for (y = 0;y < copy->height;y++)
					{
						for (x = 0;x < copy->width;x++)
						{
							((UINT16 *)copy->line[copy->height - y -1])[x] = ((UINT16 *)bitmap->line[sy+(x/scalex)])[sx +(y/scaley)];
						}
					}
					break;
				case 32:
					for (y = 0;y < copy->height;y++)
					{
						for (x = 0;x < copy->width;x++)
						{
							((UINT32 *)copy->line[copy->height - y -1])[x] = ((UINT32 *)bitmap->line[sy+(x/scalex)])[sx +(y/scaley)];
						}
					}
					break;
				default:
...
void save_screen_snapshot(struct mame_bitmap *bitmap)
{
	...
	if (mame_faccess(name,FILETYPE_SCREENSHOT))
	{
		...
		{
#ifdef GCW0
			/* otherwise use "gamenameNN.png" */
			sprintf(name,"%s%02d",Machine->gamedrv->name,snapno++);
#else
