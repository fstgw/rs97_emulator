...
INLINE UINT16 rgb_to_direct15(rgb_t rgb)
{
	return  (  RGB_RED(rgb) >> 3) * (direct_rgb_components[0] / 0x1f) +
			(RGB_GREEN(rgb) >> 2) * (direct_rgb_components[1] / 0x3f) +
...
int palette_start(void)
{
	...
	switch (colormode)
	{
		case DIRECT_15BIT: printf("colormode=DIRECT_15BIT\n"); break;
		case DIRECT_32BIT: printf("colormode=DIRECT_32BIT\n"); break;
		case PALETTIZED_16BIT: printf("colormode=PALETTIZED_16BIT\n"); break;
		case (DIRECT_15BIT | DIRECT_32BIT): printf("colormode=DIRECT_RGB\n");break;
	}
#endif
