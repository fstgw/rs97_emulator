...
{
#ifdef GCW0
	{ IPT_UI_CONFIGURE,         "Config Menu",			SEQ_DEF_2(KEYCODE_TAB, KEYCODE_ESC) },
#else
...
extern void ui_displaymenu(struct mame_bitmap *bitmap, const char **items, const char **subitems, char *flag, int selected, int arrowize_subitem);
...
int autofire_menu(struct mame_bitmap *bitmap, int selected)
{
