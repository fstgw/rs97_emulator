...
void initSDL(int window) {
  ...
  if ( (video = SDL_CreateRGBSurface(SDL_SWSURFACE, SCREEN_WIDTH, SCREEN_HEIGHT, videoBpp, 0, 0, 0, 0)) == NULL ) {
...
void flipScreen() {
  ...
  //SDL_Flip(video);
  SDL_Surface *p = SDL_ConvertSurface(video, ScreenSurface->format, 0);
