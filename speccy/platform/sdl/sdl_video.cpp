...
bool InitVideo()
{
    ...
    screen = SDL_CreateRGBSurface(SDL_SWSURFACE, SCREEN_WIDTH, SCREEN_HEIGHT, BPP, 0, 0, 0, 0);
#else
...
void UpdateScreen()
{
	...
	if (gcw_fullscreen_current != gcw_fullscreen)
	{
		...
		if(gcw_fullscreen)
		{
			...
			ScreenSurface = SDL_SetVideoMode(320, 480, BPP, SDL_HWSURFACE);
      screen = SDL_CreateRGBSurface(SDL_SWSURFACE, SCREEN_WIDTH - 2 * BORDER_WIDTH, SCREEN_HEIGHT - 2 * BORDER_HEIGHT, BPP, 0, 0, 0, 0);
		}
		else
		{
			//screen = SDL_SetVideoMode(SCREEN_WIDTH, SCREEN_HEIGHT, BPP, SDL_HWSURFACE|SDL_TRIPLEBUF);
			ScreenSurface = SDL_SetVideoMode(30, 480, BPP, SDL_HWSURFACE);
      screen = SDL_CreateRGBSurface(SDL_SWSURFACE, SCREEN_WIDTH, SCREEN_HEIGHT, BPP, 0, 0, 0, 0);
		}
	}
#endif
