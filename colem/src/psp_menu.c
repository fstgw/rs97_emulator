...
psp_main_menu_reset(void)
{
  ...
  psp_sdl_back2_print( 90, 110, "Reset Coleco !", PSP_MENU_WARNING_COLOR);
  psp_sdl_flip();
  emulator_reset();
  sleep(1);
}
...
psp_main_menu_load_slot()
{
  ...
  error = cv_snapshot_load_slot(cur_slot);
...
{
  int error;
  error = cv_snapshot_save_slot(cur_slot);
...
psp_main_menu_del_slot()
{
  ...
  error = cv_snapshot_del_slot(cur_slot);
...
psp_main_menu_editor()
{
  ...
  sprintf(TmpFileName, "%s/txt/%s.txt", CV.cv_home_dir, CV.cv_save_name);
  ...
  while (! end_menu)
  {
        ...
        (new_pad == GP2X_CTRL_CIRCLE))
    {
      ...
      switch (cur_menu_id ) 
      {
        ...
        case MENU_LOAD_STATE : if (psp_main_menu_load(FMGR_FORMAT_STATE)) {
                                 end_menu = 1;
                               }
                               old_pad = new_pad = 0;
        break;              
