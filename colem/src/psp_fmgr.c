...
psp_fmgr_getExtId(const char *szFilePath) 
{
  ...
  if((pszExt = strrchr(szFilePath, '.'))) {
    if ((!strcasecmp(pszExt, ".rom")) ||
        (!strcasecmp(pszExt, ".col"))) {
      return FMGR_FORMAT_ROM;
    } else 
    if ((!strcasecmp(pszExt, ".sta")) ||
        (!strcasecmp(pszExt, ".stz"))) {
      return FMGR_FORMAT_STATE;
    } else
    if (!strcasecmp(pszExt, ".kbd")) {
      return FMGR_FORMAT_KBD;
    } else
    if (!strcasecmp(pszExt, ".joy")) {
      return FMGR_FORMAT_JOY;
    } else
    if (!strcasecmp(pszExt, ".cht")) {
...
static char user_filedir_save[GP2X_FMGR_MAX_NAME];
  ...
  if (psp_file_request(user_filename, user_filedir)) {
    ...
    else 
    {
      ...
      if (file_format == FMGR_FORMAT_ZIP) {
        ...
        if (user_file_format == FMGR_FORMAT_ROM) /* Rom */ error = cv_load_rom(user_filename, 1);
        else error = 1;
      }
      else 
      {
        if (file_format == FMGR_FORMAT_ROM) /* Rom */ error = cv_load_rom(user_filename, 0);
        else
        if (file_format == FMGR_FORMAT_STATE) /* State */ error = cv_load_state(user_filename);
        else
        ...
        if (file_format == FMGR_FORMAT_SET) /* Settings */  error = cv_load_file_settings(user_filename);
        else
        if (file_format == FMGR_FORMAT_CHT) /* Cheat */  error = cv_load_file_cheat(user_filename);
      }
    }
