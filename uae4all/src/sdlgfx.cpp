...
#endif
{
  ...
  int x, y;
  uint32_t *s = (uint32_t*)prSDLScreen->pixels;
  ...
  SDL_UpdateRect(ScreenSurface, 0, 0, 320, 480);
...
static void graphics_subinit (void)
{
	...
	if (prSDLScreen==NULL)
#ifdef DREAMCAST
		prSDLScreen = SDL_SetVideoMode(320, 480, 16, uiSDLVidModFlags|VIDEO_FLAGS);
#else
	...
	else
	{
		...
		memset(ScreenSurface->pixels, 0, 320 * 480 * prSDLScreen->format->BytesPerPixel);
#if !defined(DREAMCAST) && !defined(DINGOO)
		if (SDL_MUSTLOCK(prSDLScreen))
			SDL_UnlockSurface(prSDLScreen);
#endif
  ...
  if(SDL_MUSTLOCK(ScreenSurface)) SDL_LockSurface(ScreenSurface);
  int x, y;
  uint32_t *s = (uint32_t*)prSDLScreen->piels;
  uint32_t *d = (uint32_t*)ScreenSurface->piels;
  for(y=0; y<240; y++){
    for(x=0; x<160; x++){
      *d++ = *s++;
    }    
 ...
 */
		...
		if (cuenta>MAX_AUTOEVENTS)
		{
			...
			for(i=0;i<0x10000;i+=78)
			{
				...
				//SDL_Flip(prSDLScreen);
				SDL_FillRect(ScreenSurface, NULL, i);
        SDL_Flip(ScreenSurface);
        SDL_Flip(ScreenSurface);
			}
			SDL_Delay(333);
